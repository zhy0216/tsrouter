...
export class Router<CTX> {
  ...
  match(url: string, method: string): ((ctx: CTX) => Promise<Response | unknown>) | undefined{
    const segments = url.split("/")
    const trie: Trie<StoreTrieData> = this.getOrNewTireByMethod(method)
    const parameterNames: string[] = []
    let curTire: Trie<StoreTrieData> | undefined = trie
    for(const segment of segments) {
      const dynamicTrie = trie.get(":")
      curTire = trie.get(segment)
      if(!curTire) {
        if(!dynamicTrie) {
          return
        } else {
          parameterNames.push(segment)
          curTire = dynamicTrie
        }
      } else {
        if(dynamicTrie) {
